...
def main():
    # File path
    file_path = r"C:\Users\Jingwen TONG\Desktop\我的文档\02_项目_202301\16-WirelessAgent-ChinaCom\Simulations\WirelessAgent_V1\Knowledge_Base\HKUST_F.osm"
    ...
    try:
        ...
        rx_positions = generate_rx_positions(bounds, 200, cartesian_buildings)
        
        # Perform ray tracing
        results = perform_ray_tracing(tx_position, rx_positions, cartesian_buildings)
        
        # Save results to CSV
        output_path = 'ray_tracing_results_test.csv' # Output file path modifed by Jingwen TONG
        ...
        # Plot TX position
        plt.scatter(tx_position[0], tx_position[1], color='red', s=150, marker='^', label='TX')
        ...
        if los_rx:
            ...
            sc_los = plt.scatter(los_x, los_y, c=los_snr, cmap='viridis', s=30, marker='o', label='LOS RX')
        ...
        if nlos_rx:
            ...
            sc_nlos = plt.scatter(nlos_x, nlos_y, c=nlos_snr, cmap='viridis', s=30, marker='x', label='NLOS RX')
        ...
        plt.xlabel('X (m)')
        plt.ylabel('Y (m)')
        plt.title('Ray Tracing Results - OpenStreetMap')
        plt.legend()
        plt.axis('equal')
